...
export async function addKeyToSong(songId: string, keyId: string): Promise<void> {
  const supabase = createClient()
  const { error } = await supabase.from("song_to_keys").insert({ song_id: songId, key_id: keyId }).select()
...
export async function setPrimarySongKey(songId: string, keyId: string): Promise<void> {
  ...
  // ตั้งค่าคีย์หลัก
  const { error } = await supabase.from("songs").update({ primary_key_id: keyId }).eq("id", songId)
...
export async function removeKeyFromSong(songId: string, keyId: string): Promise<void> {
  ...
  const { data: song } = await supabase.from("songs").select("primary_key_id").eq("id", songId).single()
  ...
  // ลบคีย์ออกจากเพลง
  const { error } = await supabase.from("song_to_keys").delete().eq("song_id", songId).eq("key_id", keyId)
...
export async function createKey(keyName: string): Promise<SongKey> {
  const supabase = createClient()
  const { data, error } = await supabase.from("song_keys").insert({ key_name: keyName }).select().single()
