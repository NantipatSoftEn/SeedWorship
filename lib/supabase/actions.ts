...
export async function getSongKeys(): Promise<SongKey[]> {
  ...
  const { data, error } = await supabase.from("song_keys").select("*").order("key_name", { ascending: true })
...
export async function addSongKey(songId: string, keyId: string): Promise<void> {
  ...
  if (!existingRelation) {
    const { error } = await supabase.from("song_to_keys").insert({ song_id: songId, key_id: keyId })
