...
export function ChordDetector({ lyrics, currentKey, onKeyChange, className }: ChordDetectorProps): JSX.Element {
  ...
  const analyzeKey = (chords: string[]): string | null => {
    ...
    for (const [key, keyChordList] of Object.entries(keyChords)) {
      if (firstChord === key) {
        ...
        const otherKeyChords = keyChordList.slice(1).map((chord) => chord.replace("m", "").replace("dim", ""))
      ...
      {detectedChords.length > 0 ? (
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300">คีย์ที่มีคอร์ด:</h4>
                          ...
                          {allKeys.map((key) => (
                            <SelectItem
                              ...
                              className={cn(
                                ...
                                Object.keys(keysWithChords).includes(key) && "text-gray-400 dark:text-gray-600",
                              )}
                              disabled={Object.keys(keysWithChords).includes(key)}
                            >
                              {key} {Object.keys(keysWithChords).includes(key) && "(มีแล้ว)"}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    ...
                    <div className="text-sm text-gray-600 dark:text-gray-400">
                      ...
                      <div className="flex flex-wrap gap-1 mt-2">
                        {detectedChords.map((chord) => (
                          <Badge key={chord} className="bg-blue-50 text-blue-700 dark:bg-blue-900 dark:text-blue-300">
                            {chord}
                          </Badge>
                        ))}
                      </div>
            ...
            <div className="bg-gray-50 dark:bg-gray-900 p-3 rounded-md border border-gray-100 dark:border-gray-800">
              {Object.keys(keysWithChords).length > 0 ? (
                  ...
                  {Object.entries(keysWithChords).map(([key, chords]) => (
                    <div key={key} className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {key}
                          {key === currentKey && <Check className="ml-1 h-3 w-3" />}
                        </Badge>
                        <span className="text-xs text-gray-500 dark:text-gray-400">({chords.length} คอร์ด)</span>
                      </div>
                ...
                <div className="text-center text-sm text-gray-500 dark:text-gray-400 py-2">ยังไม่มีคีย์ที่กำหนด กรุณาเพิ่มคีย์</div>
              )}
            </div>
          </div>
